...
print "  %5s    %20s    %8s     %s       %-42s   %s" % ("Size", "Address    ", "HexSize", "Type", "Symbol", "Path")
...
#  Skip everything till we get to interesting point of map
  if string.find(line, "Linker script and memory map") != -1:
      ...
      if len(words) > 1:
        ...
        if (Size > 0):
          ...
          if SymType != SYM_TYPE_UNKNOWN:
            print "  %5d    0x%016X   %8s   %8s   %-40s   %s" % (Size, Addr, words[2], TypeStrings[SymType], prevsym, file)
      ...
      if len(words) == 4:
        ...
        if (Size > 0):
          if SymType == SYM_TYPE_TEXT:
            ...
            DataTotal = DataTotal + Size
          if SymType != SYM_TYPE_UNKNOWN:
            print "  %5d    0x%016X   %8s   %8s  %40s    %s" % (Size, Addr, words[2], TypeStrings[SymType], '', file)
          Addr = 0x3434
        ...
        if (Size > 0):
          if SymType == SYM_TYPE_TEXT:
            ...
            DataTotal = DataTotal + Size
          if SymType != SYM_TYPE_UNKNOWN:
            print "  %5d    0x%016X   %8s   %8s   %-40s   %s" % (Size, Addr, words[1], TypeStrings[SymType], prevsym, file)
