...
-------------------------------------------------------------------------------
  ...
  $Header: //components/rel/build.glue/1.0/lib/meta_lib.py#12 $
  $DateTime: 2015/10/05 16:11:29 $
...
Attributes, if any, are not returned.'''
   ...
   def __init_fb_obj(self):
      ...
      if self.FINDABLE:
         ...
         self.fb = fb_info_lib.fbInfo(self.on_linux, self.logger)
...
def ParsePartitionFileForFilenames(rawprogram_xml, element_tags=["program","patch"]):
   ...
   # Iterate
   rawprogram_files_list = []
   # Work through the rawprogram file looking for filename attr
   for element in iter:
      if element.tag in element_tags:
         # Next the attributes (available on the instance itself using Python dictionary protocol)
         if element.keys():
            for name, value in element.items():
               if name=="filename":
                  TheFilename = value.strip()   # many filename=""
                  if len(TheFilename)>0:
                     ##print "\t\tName: '%s', Value: '%s'"%(name, value)
                     rawprogram_files_list.append(TheFilename)   # actual filename ex. "sbl1.mbn"
                     break
