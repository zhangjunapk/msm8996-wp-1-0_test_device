...
class complex_general_properties(GeneratedsSuper):
    ...
    def exportChildren(self, outfile, level, namespace_='tns:', name_='complex_general_properties', fromsubclass_=False):
        ...
        if self.hmac is not None:
            showIndent(outfile, level)
            outfile.write('<%shmac>%s</%shmac>\n' % (namespace_, self.gds_format_string(quote_xml(self.hmac).encode(ExternalEncoding), input_name='hmac'), namespace_))
    ...
    def hasContent_(self):
        if (
            ...
            self.activation_enablement is not None
            ):
            return True
        else:
            return False
    ...
    def exportLiteralChildren(self, outfile, level, name_):
        ...
        if self.hmac is not None:
            showIndent(outfile, level)
            outfile.write('hmac=%s,\n' % quote_python(self.hmac).encode(ExternalEncoding))
        if self.UIE_key is not None:
            showIndent(outfile, level)
    ...
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'selected_signer':
            ...
            hmac_ = child_.text
            hmac_ = self.gds_validate_string(hmac_, node, 'hmac')
            self.hmac = hmac_
