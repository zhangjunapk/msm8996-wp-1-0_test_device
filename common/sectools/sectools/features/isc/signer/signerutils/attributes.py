...
class AttestationCertificate(Certificate):
    ...
    def process_ou_field(self, ou_str):
        ...
        if ou_number == '16':
            ...
            if match is None:
                raise RuntimeError('OU field OU=' + ou_str + ' is incorrectly formatted')
            ou_value = match.group(2) + ' ' + match.group(3) + ' ' + match.group(4) + ' ' + match.group(5) + ' ' + match.group(6) + ' ' + match.group(7)
            ou_id = match.group(8)
...
class SigningAttributes(object):
    ...
    def compare(self, other, exclude=[]):
        ...
        for attr in self.ATTR_STR_DICT:
            ...
            if attr == 'debug_serials':
                serials_match, cert_errors, config_errors = self.compare_lists(other, attr, True, True)
                if not serials_match:
                    mismatch.append(('sn', cert_errors, config_errors))
                continue
