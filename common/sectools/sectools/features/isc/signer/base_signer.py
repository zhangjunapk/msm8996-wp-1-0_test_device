...
def ishmac(imageinfo):
    return not imageinfo\
        ...
        or not imageinfo.general_properties.hmac\
        or imageinfo.general_properties.hmac.lower() == 'true'
...
class BaseSigner(SecPluginIntf_Abs):
    ...
    def sign(self, binary_to_sign, imageinfo, debug_dir=None, is_hash=False):
        ...
        if is_hash:
            ...
            logger.info('Using ' + ('QC HMAC' if is_hmac else 'SHA256') + ' for hash segment')
    ...
    def validate(self, image, root_cert_hash=None, imageinfo=None):
        if image.is_signed():
            ...
            is_hmac = ishmac(imageinfo)
            use_pss = imageinfo and imageinfo.general_properties and imageinfo.general_properties.rsa_padding and imageinfo.general_properties.rsa_padding.lower() == 'pss'
            if not self.validate_sig(image.data_to_sign, image.data_signature, cert_chain_der, hash_algo='qcom_hmac' if is_hmac else 'sha256', use_pss=use_pss):
    ...
    def validate_sig(self, to_sign, signature, cert_chain_der, hash_algo=None, use_pss=False):
        ...
        is_hmac = not hash_algo or hash_algo == 'qcom_hmac'
        ...
        image_hash = Hasher().get_hash(to_sign,
                                       hmac_params=hmac_params if is_hmac else None,
                                       sha_algo=hash_algo)

        public_key = crypto_functions.get_public_key_from_cert_chain(cert_chain_pem)
        decrypted_hash = crypto_functions.decrypt_with_public_key(signature, public_key, image_hash, use_pss)
