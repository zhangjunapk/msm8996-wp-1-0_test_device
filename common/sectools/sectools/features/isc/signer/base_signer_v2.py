...
class BaseSignerV2(BaseSigner):
    ...
    PAD_PSS_HASH_SHA2 = cert_functions.HASH_ALGO_SHA2
    ...
    def set_input(self, data_to_sign, imageinfo, debug_dir=None, is_hash=False):
        ...
        # Set the padding
        self.padding = (self.PAD_PSS if (sa.rsa_padding and sa.rsa_padding.lower() == 'pss') else
                        self.PAD_PKCS)
        ...
        if cert_property.priv_path and c_path.validate_file(cert_property.priv_path):
            ...
            with open(cert_property.priv_path, 'rb') as fp:
                ...
                if rsa_functions.get_key_format(priv_key) != utils.FORMAT_PEM:
                    priv_key = rsa_functions.get_key_in_format(priv_key,
                                                               utils.FORMAT_DER,
                                                               utils.FORMAT_PEM)
    ...
    def get_signature(self):
        ...
        signature = self.sign_hash(self.hash_to_sign, self.certs[self.ATTEST].priv_key, padding=self.padding)

        # Return the generated signature
        return signature

    def sign_hash(self, hashbin, priv_key, padding=PAD_PKCS):
        return rsa_functions.sign(hashbin, priv_key, padding=padding,
                                  hash_algo=self.PAD_PSS_HASH_SHA2, salt_len=self.PAD_PSS_SALT_1)
    ...
    def _get_cert_key(self, cert_type):
        ...
        cert, cert_type_obj.priv_key, cert_type_obj.pub_key = \
            self.generate_new_cert(cert_type, cert_type_obj,
                                   prev_type_obj, extfile=cert_type_obj.extfile,
    ...
    def generate_new_cert(self, cert_type, cert_info, prev_cert_info,
        ...
        cert_req, priv_key, pub_key = self.generate_cert_request(cert_type, cert_info,
                                                                 subject_params,
                                                                 self_sign=self_sign,
    ...
    def generate_cert_request(self, cert_type, cert_info, subject_params,
                              self_sign=False, key_exp=None, key_size=None,
                              padding=PAD_PKCS):
        # Create a new key if there isnt one
        ...
        cert = cert_functions.create_cert(cert_info.priv_key,
                                          ...
                                          pad_hash_algo=self.PAD_PSS_HASH_SHA2)

        return cert, cert_info.priv_key, cert_info.pub_key

    def generate_key_pair(self, exponent, keysize):
        return rsa_functions.gen_keys(exponent, keysize)

    def sign_cert_request(self, cert_req, prev_cert_info, extfile=None,
                          padding=PAD_PKCS):
        return cert_functions.sign_cert(cert_req, prev_cert_info.cert,
    ...
    def create_subject_params_attest(self, in_params):
        ...
        certificate_hash_alg = SHA1_OU_STRING if self.signing_attributes.hash_algorithm == 'sha1' else SHA256_OU_STRING
