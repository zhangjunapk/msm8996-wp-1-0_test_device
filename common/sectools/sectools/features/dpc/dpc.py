...
class DebugPolicyCore(FeatureBase_Abs):
    ...
    def _validate_sign(self, dp_file, rootcerthash=None):
    ...
    def validate(self, rootcerthash=None):
        """Validate the given debugpolicy elf file.
        """
        self._validate_sign(self.input_file_list, rootcerthash)
        logger.info('Validating debugpolicy file against debug policy config file..')
        self._validate_generation(self.input_file_list[0])

    def process(self, verify_setup=False, generate=False, sign=False, validate=False, rch=None,
