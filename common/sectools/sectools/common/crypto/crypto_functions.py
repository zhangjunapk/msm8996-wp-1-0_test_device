...
def encrypt_with_private_key(message, private_key, use_pss=False):
    openssl_service = openssl_implementation_discovery.OpenSSL_Service()
    return openssl_service.openssl_implementation.encrypt_with_private_key(message, private_key, use_pss)

def decrypt_with_public_key(encypted_message, public_key, image_hash, use_pss=False):
    openssl_service = openssl_implementation_discovery.OpenSSL_Service()
    return openssl_service.openssl_implementation.decrypt_with_public_key(encypted_message, public_key, image_hash, use_pss)
...
def create_root_certficate(root_cert_params, root_key_pair, days, configfile, serial_num, use_pss):
    openssl_service = openssl_implementation_discovery.OpenSSL_Service()
    return openssl_service.openssl_implementation.create_root_certficate(root_cert_params, root_key_pair, days, configfile, serial_num, use_pss)

def create_certificate(certificate_params, certificate_key_pair, CACertificate, CA_key_pair, days=7300, configfile="opensslroot.cfg", serial_num=1, extfile_name="v3.ext", use_pss=False):
    openssl_service = openssl_implementation_discovery.OpenSSL_Service()
    return openssl_service.openssl_implementation.create_certificate(certificate_params, certificate_key_pair, CACertificate, CA_key_pair, days, configfile, serial_num, extfile_name, use_pss=use_pss)
...
def cert_uses_pss(certificate):
    openssl_service = openssl_implementation_discovery.OpenSSL_Service()
    return openssl_service.openssl_implementation.cert_uses_pss(certificate)
