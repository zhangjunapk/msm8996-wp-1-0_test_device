...
class CertOpenSSLImpl(CertBase):
    ...
    def create_cert(self, priv_key, subject_params=None, config=None,
                    ...
                    padding=CertBase.PAD_PKCS, pad_hash_algo=CertBase.HASH_ALGO_SHA2,
                    pad_salt_len=CertBase.PAD_PSS_SALT_20):
        ...
        try:
            cmd = [self.openssl, 'req', '-new', '-sha256', '-key', priv_path]
    ...
    def sign_cert(self, cert, ca_cert, ca_priv_key, days=None, serial=None,
                  extfile=None, padding=CertBase.PAD_PKCS, pad_hash_algo=CertBase.HASH_ALGO_SHA2,
                  pad_salt_len=CertBase.PAD_PSS_SALT_20):
        ...
        try:
            ...
            cmd = [self.openssl, 'x509', '-req', '-sha256',
                   '-in', cert_path,
                   '-CA', ca_cert_path,
                   '-CAkey', ca_priv_path,
                   ]
    ...
    def validate_cert_chain(self, root_cert, ca_cert_chain):
        ...
        try:
            ...
            if ca_cert_chain_path is not None: c_path.remove_tmp_file(ca_cert_chain_path)
            if root_cert_path is not None: c_path.remove_tmp_file(root_cert_path)
