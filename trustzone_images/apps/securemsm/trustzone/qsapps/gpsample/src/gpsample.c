  ...
  void)
{
  ...
  do
  {
    ...
    result = TEE_GetOperationInfo(operation, &operationInfo);
    if(TEE_SUCCESS != result)
    {
      GPLOG_FATAL2("TEE_GetOperationInfo() failed", result);
      TEE_FreeOperation(operation);
      break;
    }

    result = TEE_DigestUpdate(operation, sha_plain_text_1, sizeof(sha_plain_text_1) - 1);
    if(TEE_SUCCESS != result)
    {
      GPLOG_FATAL2("TEE_DigestUpdate() failed", result);
      TEE_FreeOperation(operation);
      return result;
    }
