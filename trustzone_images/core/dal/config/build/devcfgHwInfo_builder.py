...
#
# Copyright (c) 2011,2012,2014 by QUALCOMM, Technologies Inc.
# All Rights Reserved.
...
# --------   ---     ---------------------------------------------------------
# 01/15/16	 ap		 Added fix for Multicfg
# 09/12/14   aa      Adding scons builder to support for auto generating dalconfig  
#                    system xml file.
...
def devcfg_parse_chip_info_header_file(ddi_file):
    ...
    while(1):
       ...
       line = line.strip()
       if(re.search( r'CHIPINFO_ID', line)):    
       ...
       if(re.search( r'ChipInfoIdType', line)):
         break
...
def devcfg_create_seg_c_file(prop_info_file, ResultantHWInfoDict, TCSRChipInfoDDIDict, tcsr_soc_hw_ver_addr, config):
   ...
   SEG_CFILE.write("#include \"DALSysTypes.h\" \n\n")
...
def create_seginfo_dict(env, config):
   ...
   for elem in devcfg_xml_tag_list:
      segment_base_addr = 'DALPROP_PropsInfo_' + str(elem) + config
...
def devcfg_create_prop_info_builder(target, source, env):
   ...
   HWInfoDict = create_seginfo_dict(rootenv, config)
