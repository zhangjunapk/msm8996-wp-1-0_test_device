...
/** TUI dialog return values, that is, status and error codes for the GP screens API. */
typedef enum {
...
/**
  ...
  @param[in]   focused_input_name     Name of the currently focused input object
                                      in the screen.
  @param[out]  final_status           A status to be returned in
                                      qsee_tui_dialog() in case of failure
                                      during the callback.
  ...
  None.
*/
typedef sec_touch_cmd_t (*qsee_tui_manage_layout_event_callback_t)(layout_mgr_err_t layout_mgr_status, const char * focused_input_name, qsee_tui_dialog_ret_val_t* final_status);
...
/** Defines a general buffer parameter.
 ...
 @note1hang If the buffer is used to represent text, the text string must be
            ...
            refer to the <i>QSEE Trusted UI Customization Tool User
...
/** Defines the general parameter set for all screens.
   ...
   make sure that the parameters match the specified layout.
   ...
   @note1hang For backward compatibility, you can pass NULL for the
               ...
               (refer to the <i>QSEE Trusted UI Customization
   ...
   make sure that a corresponding layout exists in the application.
 */
typedef struct qsee_tui_dialog_params_s {
    ...
    /** Union of the parameters of the different screen types. @newpagetable */
	union {
