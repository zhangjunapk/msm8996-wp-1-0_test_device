...
/*===========================================================================
  ...
  $Header: //components/rel/core.tz/1.0.3.c1/securemsm/trustzone/qsapps/sampleapp/src/app_crypto.c#1 $
...
/* TODO: To use this cases must have them in hex string format and then read into bigval via qsee_GENERIC_ECC_convert_input_to_bigval and then back to binary with bigval_to_binary */

//static uint8 ecc_224_public_key_x[] = "4c741e4d20103670b7161ae72271082155838418084335338ac38fa4";
//
//static uint8 ecc_224_public_key_y[] = "db7919151ac28587b72bad7ab180ec8e95ab9e2c8d81d9b9d7e2e383";
//
//static uint8 ecc_224_private_key[] = "888fc992893bdd8aa02c80768832605d020b81ae0b25474154ec89aa";
//
//static uint8 ecc_224_signature[] = "0909c9b9cae8d2790e29db6afdb45c04f5b072c4c20410c7dc9b6772" // r
...
//static uint8 ecc_224_hash[] = "00c6fc53c1986d19a8a8b580ee553dc1240745d760647d1c0adf442c133c7f56";
//
//static uint8 ecc_384_public_key_x[] = "0400193b21f07cd059826e9453d3e96dd145041c97d49ff6b7047f86bb0b0439e909274cb9c282bfab88674c0765bc75";
//
//static uint8 ecc_384_public_key_y[] = "f70d89c52acbc70468d2c5ae75c76d7f69b76af62dcf95e99eba5dd11adf8f42ec9a425b0c5ec98e2f234a926b82a147";
//
//static uint8 ecc_384_private_key[] = "c602bc74a34592c311a6569661e0832c84f7207274676cc42a89f058162630184b52f0d99b855a7783c987476d7f9e6b";
//
//static uint8 ecc_384_signature[] = "b11db00cdaf53286d4483f38cd02785948477ed7ebc2ad609054551da0ab0359978c61851788aa2ec3267946d440e878\
...
//static uint8 ecc_384_hash[] = "bbbd0a5f645d3fda10e288d172b299455f9dff00e0fbc2833e18cd017d7f3ed1";
//
//static uint8 ecc_521_public_key_x[] = "007d042ca19408524e68b981f1419351e3b84736c77fe58fee7d11317df2e850d960c7dd10d10ba714c8a609d163502b79d682e8bbecd4f52591d2748533e45a867a";
//
//static uint8 ecc_521_public_key_y[] = "0197ac6416111ccf987d290459ebc8ad9ec56e49059c992155539a36a626631f4a2d89164b985154f2dddc0281ee5b5178271f3a76a0914c3fcd1f97be8e8376efb3";
//
//static uint8 ecc_521_private_key[] = "01e8c05996b85e6f3f875712a09c1b40672b5e7a78d5852de01585c5fb990bf3812c3245534a714389ae9014d677a449efd658254e610da8e6cad33414b9d33e0d7a";
//
//static uint8 ecc_521_signature[] = "009dd1f2a716843eedec7a6645ac834d4336e7b18e35701f06cae9d6b290d41491424735f3b57e829ad5de055eaeef1778f051c1ee152bf2131a081e53df2a567a8a\
...
//uint8 ecc_521_hash[] = "53e6537cb6ea68ae47a81611c22756d770d7a37e336c3af0b0814b04fa39434b";
...
static aes_vector_type_t aes_test_vectors[] =
{
...
static aes_vector_t_2 aes_test_vectors_2 =
{
//  QSEE_CIPHER_ALGO_AES_128, QSEE_CIPHER_MODE_CBC, NULL, NULL,
//  fourth entry in the structure is pt_len which should not be set to NULL
...
static uf_aes_vector_type_t sw_aes_test_vectors[] =
{
...
static int key_index_gcm[MAX_TESTS] =
    { 0, 0, 1, 1, 1, 1 };

static unsigned char key_gcm[MAX_TESTS][32] =
{
    { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static size_t iv_len_gcm[MAX_TESTS] =
    { 12, 12, 12, 12, 8, 60 };

static int iv_index_gcm[MAX_TESTS] =
    { 0, 0, 1, 1, 1, 2 };

static unsigned char iv_gcm[MAX_TESTS][64] =
{
    { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00 },
...
static size_t add_len_gcm[MAX_TESTS] =
    { 0, 0, 0, 20, 20, 20 };

static int add_index_gcm[MAX_TESTS] =
    { 0, 0, 0, 1, 1, 1 };

static unsigned char additional_gcm[MAX_TESTS][64] =
{
    { 0x00 },
...
static size_t pt_len_gcm[MAX_TESTS] =
    { 0, 16, 64, 60, 60, 60 };

static int pt_index_gcm[MAX_TESTS] =
    { 0, 0, 1, 1, 1, 1 };

static unsigned char pt_gcm[MAX_TESTS][64] =
{
    { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 },
...
static unsigned char ct_gcm[MAX_TESTS * 3][64] =
{
    { 0x00 },
...
static unsigned char tag_gcm[MAX_TESTS * 3][16] =
{
...
static triple_des_vector_t triple_des_test_vectors[] =
{
...
static triple_des_vector_t_2 triple_des_test_vectors_2 =
{
  QSEE_CIPHER_ALGO_TRIPLE_DES, QSEE_CIPHER_MODE_CBC, triple_des_plain_text_1_cbc, sizeof(triple_des_plain_text_1_cbc), 
...
static aes_xts_vector_t aes_xts_test_vectors[] =
{
...
// Mlen = 64
static uint8 aescmac128_plain_text_3[] =
{
  0x6b, 0xc1, 0xbe, 0xe2, 0x2e, 0x40, 0x9f, 0x96,
  0xe9, 0x3d, 0x7e, 0x11, 0x73, 0x93, 0x17, 0x2a,
  0xae, 0x2d, 0x8a, 0x57, 0x1e, 0x03, 0xac, 0x9c,
...
// Mlen = 32
static uint8 aescmac128_plain_text_4[] =
{
                           ...
                           uint8 verify)
{
  ...
  {
      QSEE_LOG(QSEE_LOG_MSG_DEBUG,"qsee_cipher_reset API failed");
      status = -E_FAILURE;
  }
  /*------------------------------------------------------------------------------
  ...
  {
    QSEE_LOG(QSEE_LOG_MSG_DEBUG,"qsee_set_bandwidth vote-on failed");
    status = -E_FAILURE;
  }
  ...
  {
    QSEE_LOG(QSEE_LOG_MSG_DEBUG,"qsee_set_bandwidth vote-off failed");
    status = -E_FAILURE;
  }
                                  ...
                                  uint32 msg_len)
{
  ...
  {  
    QSEE_LOG(QSEE_LOG_MSG_DEBUG,"qsee_hash_reset API failed");
    status = -E_FAILURE;
  }
                                ...
                                uint8 *ct2)
{
  ...
  {  
	QSEE_LOG(QSEE_LOG_MSG_DEBUG,"qsee_hash_set_param API for mode failed = 0x%x",hash_mode);
    status = -E_FAILURE;
  }
  ...
  {  
    QSEE_LOG(QSEE_LOG_MSG_DEBUG,"qsee_hash_set_param API for seq failed");
    status = -E_FAILURE;
  }
  ...
  if (E_SUCCESS == status && NULL != iv &&
      qsee_cipher_set_param(cipher_ctx, QSEE_CIPHER_PARAM_IV, iv, QSEE_AES128_IV_SIZE) < 0)
  {
    QSEE_LOG(QSEE_LOG_MSG_DEBUG,"qsee_cipher_set_parm API for IV failed");
    status = -E_FAILURE;
  }
  ...
  {
    QSEE_LOG(QSEE_LOG_MSG_DEBUG,"qsee_hashcipher_encrypt API failed");
    status = -E_FAILURE;
  }
  ...
  -------------------------------------------------------------------------*/  
  if ((E_SUCCESS == status) && (0 != memcmp(ct, ct2, ct_len)))
  {
    QSEE_LOG(QSEE_LOG_MSG_DEBUG,"qsee_hashcipher_encrypt CT mem compare failed");
    status = -E_FAILURE;
  }
  ...
  -------------------------------------------------------------------------*/  
  if ((E_SUCCESS == status) && (0 != memcmp(result_digest1, result_digest2, digest_len)))
  {
    QSEE_LOG(QSEE_LOG_MSG_DEBUG,"qsee_hashcipher_encrypt HASH mem compare failed");
    status = -E_FAILURE;
  }
  ...
  if (E_SUCCESS == status && 
      qsee_hash_set_param(hash_ctx, QSEE_HASH_PARAM_MODE, &hash_mode, sizeof(QSEE_HASH_MODE_ET)) < 0)
  {  
    QSEE_LOG(QSEE_LOG_MSG_DEBUG,"qsee_hash_set_param API for mode2 failed");
    status = -E_FAILURE;
  }
  ...
  if (E_SUCCESS == status && 
      qsee_hash_set_param(hash_ctx, QSEE_HASH_PARAM_SEQ, &hash_seq, sizeof(QSEE_HASH_SEQ_ET)) < 0)
  {  
    QSEE_LOG(QSEE_LOG_MSG_DEBUG,"qsee_hash_set_param API for seq2 failed");
    status = -E_FAILURE;
  }
  ...
  if (E_SUCCESS == status && NULL != iv &&
      qsee_cipher_set_param(cipher_ctx, QSEE_CIPHER_PARAM_IV, iv, QSEE_AES128_IV_SIZE) < 0)
  {
    QSEE_LOG(QSEE_LOG_MSG_DEBUG,"qsee_cipher_set_parm API for IV2 failed");
    status = -E_FAILURE;
  }
  ...
  {
    QSEE_LOG(QSEE_LOG_MSG_DEBUG,"qsee_hashcipher_decrypt API failed");
    status = -E_FAILURE;
  }
  ...
  -------------------------------------------------------------------------*/  
  if ((E_SUCCESS == status) && (0 != memcmp(pt, ct, pt_len)))
  {
    QSEE_LOG(QSEE_LOG_MSG_DEBUG,"qsee_hashcipher_decrypt PT mem compare failed");
    status = -E_FAILURE;
  }
  ...
  if ((E_SUCCESS == status) && (0 != memcmp(result_digest1, result_digest2, digest_len)))
  {
    QSEE_LOG(QSEE_LOG_MSG_DEBUG,"qsee_hashcipher_decrypt HASH mem compare failed");
    status = -E_FAILURE;
  }
                                ...
                                uint8 *iv)
{
  ...
  if (E_SUCCESS == status && 
      qsee_hash_set_param(hash_ctx, QSEE_HASH_PARAM_MODE, &hash_mode, sizeof(QSEE_HASH_MODE_ET)) < 0)
  {  
	QSEE_LOG(QSEE_LOG_MSG_DEBUG,"qsee_hash_set_param API failed");
    status = -E_FAILURE;
  }
...
int tz_app_hash_aes_func(void)
{
  ...
  for (idx = 0; idx < num; idx++)
  {
    ...
    for (j = 16; j <= sizeof(aes_large_plain_text2); j *= 2)
    {
	  ...
	  {
	    QSEE_LOG(QSEE_LOG_MSG_DEBUG,"Crypto Simultaneous Hash/AES Functional Test - Vect Num:%u Vect Size:%u Failed", idx, j);
        return status;
	  }
    }
  }
...
int tz_app_hash_aes_perf(void)
{
  ...
  for (size = TZBSP_TEST_CRYPTO_PERF_START_SIZE; size <= TZBSP_TEST_CRYPTO_PERF_END_SIZE; size = size*2)
  {
    ...
    for (i = 0; i < num; i++)
    {
      ...
      if (status != E_SUCCESS) 
      {
        QSEE_LOG(QSEE_LOG_MSG_DEBUG,"tz_app_crypto_hash_aes_perf test FAILED for iteration: %u", i);
        return status;
      }
    }
...
int tz_app_crypto_generic_ecdsa_func(generic_ecc_vector_type *data_ptr)
{
  ...
  do
  {
    ...
    if (rv) 
    {
      QSEE_LOG(QSEE_LOG_MSG_DEBUG,"qsee_SW_GENERIC_ECDSA_Verify API failed to verify vector signature. Error %d", rv);
      rv = -E_FAILURE;
      break;
    }
                             ...
                             uint8 verify)
{
  ...
  if (E_SUCCESS == status && (verify == 1) &&
      (ct_tmp = qsee_malloc(pt_len)) == NULL)
  {
    QSEE_LOG(QSEE_LOG_MSG_DEBUG,"tz_app_ufcrypto_aes_func ct_tmp malloc failed");
    status = -E_FAILURE;
  }
  ...
  {
    QSEE_LOG(QSEE_LOG_MSG_DEBUG,"qsee_SW_Cipher memcmp encrypt failed");
    status = -E_FAILURE;
  }
  ...
  if ((E_SUCCESS == status) && (verify == 1) && 
      (pt_tmp = qsee_malloc(pt_len)) == NULL)
  {
    QSEE_LOG(QSEE_LOG_MSG_DEBUG,"tz_app_ufcrypto_aes_func pt_tmp malloc failed");
    status = -E_FAILURE;
  }
  ...
  {  
    QSEE_LOG(QSEE_LOG_MSG_DEBUG,"qsee_SW_Cipher memcmp decrypt failed");
    status = -E_FAILURE;
  }
                                 ...
                                 uintnt *auth_struct)
{
  ...
  if ((E_SUCCESS == status))
  {
    if(pt[2] == 0 || pt[3] == 1)
    {
      ...
      {
        //Now compare encrypted results
        if (memcmp((void*)((uintnt)pt[1]), (void*)((uintnt)ptr_tmp[1]), ct_len) != 0)
        {
          QSEE_LOG(QSEE_LOG_MSG_DEBUG,"encrypted data compare failed");
          status = -E_FAILURE;
        }

        if(memcmp(tag_auth, (void*)((uintnt)auth_struct[0]), auth_struct[1]) != 0)
        {
    ...
    ---------------------------------------------------------------------------------------*/
      ...
      {
        //Now compare decrypted results
        if (memcmp((void*)((uintnt)pt[0]), (void*)((uintnt)ptr_tmp[0]), pt_len) != 0)
        {
                              ...
                              uint8 *digest)
{
  ...
  do
  {
    ...
    //Compare results
    if (0 != memcmp(result_digest, digest, digest_len))
    {
      status = -E_FAILURE;
    }
  } while (0);
