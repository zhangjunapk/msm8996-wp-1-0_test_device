...
int32_t tee_i2c_open(int32_t deviceId)
{
  ...
  /* Take ownership of the GPIOs associated with the device */
  if ((ret = II2C_acquireExclusiveAccess(g_tee_i2c_obj, 
                                         deviceId)) == Object_OK)
  {
    if ((ret = II2C_open(g_tee_i2c_obj, 
                         deviceId)) != Object_OK) 
    {
      (void)II2C_releaseExclusiveAccess(g_tee_i2c_obj, 
                                        deviceId);
    }
  }
